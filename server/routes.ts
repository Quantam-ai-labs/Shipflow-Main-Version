...
import { aiInsightCache, robocallLogs } from "@shared/schema";
import {
...
export async function registerRoutes(
  ...
  app.get("/api/robocall/settings", isAuthenticated, async (req: any, res) => {
    try {
      ...
      res.json({
        ...
        apiKey: merchant.robocallApiKey || "",
        configured: !!(merchant.robocallEmail && merchant.robocallApiKey),
      });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.put("/api/robocall/settings", isAuthenticated, async (req: any, res) => {
    try {
      const merchantId = await requireMerchant(req, res);
      if (!merchantId) return;
      const { email, apiKey } = req.body;
      if (!email || !apiKey) return res.status(400).json({ error: "Email and API key are required" });
      const verifyData = await safeFetchJson(`${ROBOCALL_API_BASE}/balance?email=${encodeURIComponent(email)}&key=${encodeURIComponent(apiKey)}`);
      if (verifyData.raw || !verifyData.sms || verifyData.sms.code !== "000") {
        return res.status(400).json({ error: "Invalid credentials. Could not verify with BrandedSMS." });
      }
      await storage.updateMerchant(merchantId, {
        robocallEmail: email,
        robocallApiKey: apiKey,
      });
      res.json({ success: true, balance: verifyData.sms.remaining_balance });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  });

  app.post("/api/robocall/send-bulk-orders", isAuthenticated, async (req: any, res) => {
    try {
      const merchantId = await requireMerchant(req, res);
      if (!merchantId) return;
      const { orderIds, voiceId } = req.body;
      if (!Array.isArray(orderIds) || orderIds.length === 0) return res.status(400).json({ error: "orderIds array is required" });
      if (orderIds.length > 50) return res.status(400).json({ error: "Maximum 50 calls per batch" });

      const merchant = await storage.getMerchant(merchantId);
      if (!merchant?.robocallEmail || !merchant?.robocallApiKey) {
        return res.status(400).json({ error: "RoboCall credentials not configured. Please set them up in Settings." });
      }
      ...
      const orderMap = new Map(orderResults.map(o => [o.id, o]));
      const results: any[] = [];

      for (const orderId of orderIds) {
        const order = orderMap.get(orderId);
        if (!order) {
          results.push({ orderId, error: "Order not found", status: "error" });
          continue;
        }
        if (!order.customerPhone) {
          results.push({ orderId, orderNumber: order.orderNumber, error: "No phone number", status: "error" });
          continue;
        }
        ...
        try {
          ...
          const data = await safeFetchJson(`${ROBOCALL_API_BASE}/send-voice?${params.toString()}`);
          const sms = data.sms;
          const callId = data.data?.call_id ? String(data.data.call_id) : (sms?.call_id ? String(sms.call_id) : undefined);
          const hasError = data.raw || data.error || (sms && sms.code !== "000" && sms.code !== "200" && sms.code !== "201");
          ...
          await db.insert(robocallLogs).values({
            ...
            error: hasError ? (data.error || sms?.response || "Unknown error") : null,
          });

          if (hasError) {
            results.push({ orderId, orderNumber: order.orderNumber, error: data.error || sms?.response || "Call failed", status: "error" });
          } else {
            const tagValue = "IVR-Pending";
            const currentTags = await db.select({ tags: orders.tags }).from(orders).where(eq(orders.id, orderId));
            const existingTags = currentTags[0]?.tags || [];
            if (!existingTags.includes(tagValue)) {
              await db.update(orders).set({ tags: [...existingTags, tagValue] }).where(eq(orders.id, orderId));
            }
            results.push({ orderId, orderNumber: order.orderNumber, callId, phone: order.customerPhone, status: "initiated" });
          }

          await new Promise(resolve => setTimeout(resolve, 200));
        } catch (err: any) {
          results.push({ orderId, orderNumber: order.orderNumber, error: err.message, status: "error" });
        }
      }

      const successCount = results.filter(r => r.status === "initiated").length;
      const failedCount = results.filter(r => r.status === "error").length;
      res.json({ results, successCount, failedCount, total: results.length });

      if (successCount > 0) {
        pollRobocallStatuses(merchantId, merchant.robocallEmail, merchant.robocallApiKey, results.filter(r => r.callId));
      }
    } catch (error: any) {
      console.error("[RoboCall] Send bulk orders error:", error);
      res.status(500).json({ error: error.message });
    }
  });

  async function pollRobocallStatuses(merchantId: string, email: string, apiKey: string, callResults: Array<{ orderId: string; callId: string; orderNumber?: string }>) {
    const pendingCalls = new Map(callResults.map(r => [r.callId, r]));
    let attempts = 0;
    const maxAttempts = 30;

    const poll = async () => {
      if (pendingCalls.size === 0 || attempts >= maxAttempts) return;
      attempts++;

      for (const [callId, callInfo] of pendingCalls) {
        try {
          const data = await safeFetchJson(`${ROBOCALL_API_BASE}/get-call?email=${encodeURIComponent(email)}&key=${encodeURIComponent(apiKey)}&id=${encodeURIComponent(callId)}`);
          const statusData = data.data || data.sms;
          if (!statusData || statusData.call_status === undefined) continue;
          ...
          let statusLabel = "polling";
          if (callStatus === 0) statusLabel = "queued";
          else if (callStatus === 1) statusLabel = "ringing";
          else if (callStatus === 2) statusLabel = "answered";
          else if (callStatus === 3) statusLabel = "busy";
          else if (callStatus === 4) statusLabel = "no_answer";
          else if (callStatus === 5) statusLabel = "failed";

          await db.update(robocallLogs).set({
            status: statusLabel,
            dtmf: dtmf || null,
            updatedAt: new Date(),
          }).where(eq(robocallLogs.callId, callId));

          if (isFinal) {
            pendingCalls.delete(callId);

            if (callStatus === 2 && dtmf && callInfo.orderId) {
              try {
                if (dtmf === "1") {
                  const currentTags = await db.select({ tags: orders.tags }).from(orders).where(eq(orders.id, callInfo.orderId));
                  const existingTags = (currentTags[0]?.tags || []).filter((t: string) => t !== "IVR-Pending");
                  if (!existingTags.includes("IVR-Confirmed")) existingTags.push("IVR-Confirmed");
                  await db.update(orders).set({ tags: existingTags }).where(eq(orders.id, callInfo.orderId));
                } else if (dtmf === "2") {
                  const currentTags = await db.select({ tags: orders.tags }).from(orders).where(eq(orders.id, callInfo.orderId));
                  const existingTags = (currentTags[0]?.tags || []).filter((t: string) => t !== "IVR-Pending");
                  if (!existingTags.includes("IVR-Cancelled")) existingTags.push("IVR-Cancelled");
                  await db.update(orders).set({ tags: existingTags }).where(eq(orders.id, callInfo.orderId));
                }
              } catch (tagErr) {
                console.error("[RoboCall] Error updating order tags:", tagErr);
              }
            } else if (isFinal && callStatus !== 2 && callInfo.orderId) {
              try {
                const currentTags = await db.select({ tags: orders.tags }).from(orders).where(eq(orders.id, callInfo.orderId));
                const existingTags = (currentTags[0]?.tags || []).filter((t: string) => t !== "IVR-Pending");
                if (!existingTags.includes("IVR-NoResponse")) existingTags.push("IVR-NoResponse");
                await db.update(orders).set({ tags: existingTags }).where(eq(orders.id, callInfo.orderId));
              } catch (tagErr) {
                console.error("[RoboCall] Error updating order tags:", tagErr);
              }
          ...
          console.error(`[RoboCall] Poll error for call ${callId}:`, err);
        }
      }
  ...
  app.get("/api/robocall/logs", isAuthenticated, async (req: any, res) => {
    try {
      const merchantId = await requireMerchant(req, res);
      if (!merchantId) return;
      const logs = await db.select().from(robocallLogs)
        .where(eq(robocallLogs.merchantId, merchantId))
        .orderBy(sql`${robocallLogs.createdAt} desc`)
        .limit(100);
      res.json(logs);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
