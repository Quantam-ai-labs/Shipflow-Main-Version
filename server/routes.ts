...
export async function registerRoutes(
  ...
  app.get("/api/shopify/callback", async (req: any, res) => {
    try {
      ...
      const requestedScopes = (process.env.SHOPIFY_APP_SCOPES || 'read_orders,write_orders,read_order_edits,write_order_edits,read_fulfillments,write_fulfillments,read_inventory,read_merchant_managed_fulfillment_orders,write_merchant_managed_fulfillment_orders').split(',').map(s => s.trim());
  ...
  app.post("/webhooks/whatsapp", async (req: any, res) => {
    ...
    try {
      ...
      // This guarantees zero-drop: all events are durably stored before we ACK Meta.
      // Merchant lookups are cached per phoneNumberId; all inserts run in parallel.
      const merchantCache = new Map<string, string | null>();
      const persistTasks: Promise<import("@shared/schema").WaRawEvent | null>[] = [];
      ...
      for (const entry of (body?.entry ?? [])) {
        for (const change of (entry?.changes ?? [])) {
          ...
          // Resolve merchant once per phone_number_id, cache result within this request
          if (!merchantCache.has(phoneNumberId)) {
            let mid: string | null = null;
            if (phoneNumberId) {
              const [row] = await db.select({ id: merchants.id }).from(merchants)
                .where(eq(merchants.waPhoneNumberId, phoneNumberId)).limit(1);
              mid = row?.id ?? null;
            }
            merchantCache.set(phoneNumberId, mid);
          }
          const webhookMerchantId = merchantCache.get(phoneNumberId) ?? null;

          for (const message of (value?.messages ?? [])) {
            persistTasks.push(
              persistRawMessageEvent({
                ...
                // Log full payload so it can be recovered from server logs even if DB insert fails
                console.error(`[WhatsApp Webhook] ${requestId} - CRITICAL persist failure: ${e.message} | payload=${JSON.stringify({ message, merchantId: webhookMerchantId })}`);
                return null;
              })
            );
          }
          ...
          for (const statusUpdate of (value?.statuses ?? [])) {
            ...
            persistTasks.push(
              persistRawStatusEvent({
                merchantId: webhookMerchantId,
                waMessageId: waId,
                webhookSource: "generic",
                payload: statusUpdate,
              }).catch((e: any) => {
                console.error(`[WhatsApp Webhook] ${requestId} - CRITICAL status persist failure: ${e.message} | payload=${JSON.stringify(statusUpdate)}`);
                return null;
              })
            );
          }
  ...
  app.post("/webhooks/whatsapp/:merchantId", async (req: any, res) => {
    ...
    try {
      ...
      // All inserts launched in parallel; failures are logged with full payload for recovery.
      const persistTasks: Promise<import("@shared/schema").WaRawEvent | null>[] = [];
          ...
          for (const message of (value?.messages ?? [])) {
            persistTasks.push(
              persistRawMessageEvent({
                ...
                console.error(`[WhatsApp Webhook:${merchantId}] ${requestId} - CRITICAL persist failure: ${e.message} | payload=${JSON.stringify(message)}`);
                return null;
              })
            );
          }
          ...
          for (const statusUpdate of (value?.statuses ?? [])) {
            ...
            persistTasks.push(
              persistRawStatusEvent({
                ...
                console.error(`[WhatsApp Webhook:${merchantId}] ${requestId} - CRITICAL status persist failure: ${e.message} | payload=${JSON.stringify(statusUpdate)}`);
                return null;
              })
            );
          }
  ...
  app.get("/api/shopify/scopes", isAuthenticated, async (req, res) => {
    try {
      ...
      const requiredScopes = (process.env.SHOPIFY_APP_SCOPES || 'read_orders,write_orders,read_order_edits,write_order_edits,read_fulfillments,write_fulfillments,read_inventory,read_merchant_managed_fulfillment_orders,write_merchant_managed_fulfillment_orders').split(',').map(s => s.trim());
  ...
  app.post("/api/booking/book", isAuthenticated, async (req, res) => {
    try {
      ...
      const { generateBatchLoadsheetPdf, savePdf } = await import(
      ...
      (async () => {
        try {
          ...
          const batchPdfBuffer = await generateBatchLoadsheetPdf({
            courierName: courier === "leopards" ? "Leopards" : "PostEx",
            merchantName: merchant.name,
            createdAt: new Date().toISOString(),
            items: bgResults.filter((r) => r.success).map((r) => {
              const order = bgFetchedOrders.find((o) => o.id === r.orderId);
              const lineItems = Array.isArray(order?.lineItems) ? (order.lineItems as any[]) : [];
              const productDetails = lineItems.length > 0
                ? lineItems.map((li: any) => {
                    const name = (li.name || li.title || "Item").trim();
                    const variant = (li.variantTitle || li.variant_title) ? ` (${li.variantTitle || li.variant_title})` : "";
                    const qty = li.quantity && li.quantity > 1 ? ` x${li.quantity}` : "";
                    return `${name}${variant}${qty}`;
                  }).join(", ")
                : (order?.itemSummary || "-");
              const totalQty = lineItems.length > 0
                ? lineItems.reduce((s: number, li: any) => s + (Number(li.quantity) || 1), 0)
                : (order?.totalQuantity || 1);
              return {
                trackingNumber: r.trackingNumber || "-",
                orderNumber: r.orderNumber || "-",
                city: order?.city || "-",
                weight: Number(order?.courierWeight) || 0,
          ...
          const batchLoadsheetPath = await savePdf(batchPdfBuffer, `booking_batch_${courier}`);
    ...
    async (req, res) => {
      try {
        ...
        if (courierNorm === "leopards") {
          ...
          riderName = settings.riderName?.trim() || "";
        ...
        const { generateBatchLoadsheetPdf, savePdf } = await import("./services/pdfGenerator");

        const loadsheetItems = bookedOrders.map((o) => {
          const lineItems = Array.isArray(o.lineItems) ? (o.lineItems as any[]) : [];
          const productDetails = lineItems.length > 0
            ? lineItems.map((li: any) => {
                const name = (li.name || li.title || "Item").trim();
                const variant = (li.variantTitle || li.variant_title) ? ` (${li.variantTitle || li.variant_title})` : "";
          ...
          const totalQty = lineItems.length > 0
            ...
            : (o.totalQuantity || 1);
          return {
            trackingNumber: o.courierTracking || "-",
            orderNumber: o.orderNumber || "-",
            city: o.city || "-",
            weight: Number(o.courierWeight) || 0,
            consigneeName: o.customerName || "-",
            productDetails,
            quantity: totalQty,
            codAmount: Number(o.totalAmount) || 0,
          };
        });

        const pdfBuffer = await generateBatchLoadsheetPdf({
          courierName: courierLabel,
          courierLoadsheetId,
          merchantName: merchant.businessName || merchant.shopName || "-",
          accountNumber: settings.accountNumber?.trim() || "",
          accountCode: settings.riderCode?.trim() || "",
          riderName,
          createdAt: new Date().toISOString(),
  ...
  app.post("/api/warehouse/generate-loadsheet", warehouseAuth, async (req, res) => {
    try {
      ...
      if (courierNormWh === "leopards") {
        ...
        whCourierLoadsheetId = await registerLeopardsLoadSheet(
          trackingNums,
      ...
      const whLoadsheetItems = fetchedOrders.map((o) => {
        const lineItems = Array.isArray(o.lineItems) ? (o.lineItems as any[]) : [];
        const productDetails = lineItems.length > 0
          ? lineItems.map((li: any) => {
              const name = (li.name || li.title || "Item").trim();
      ...
      const whPdfBuffer = await generateBatchLoadsheetPdf({
        courierName: courierLabelWh,
        courierLoadsheetId: whCourierLoadsheetId,
        merchantName: merchant.businessName || merchant.shopName || "-",
        accountNumber: whSettings.accountNumber?.trim() || "",
        accountCode: whSettings.riderCode?.trim() || "",
        riderName: whRiderName,
        createdAt: new Date().toISOString(),
        items: whLoadsheetItems,
      });
