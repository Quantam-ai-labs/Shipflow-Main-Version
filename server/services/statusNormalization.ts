...
function keywordFallback(rawStatus: string): UniversalStatus | null {
  ...
  if (s.includes('delivered') && !s.includes('un')) return 'DELIVERED';
  if (s.includes('out') && s.includes('delivery')) return 'OUT_FOR_DELIVERY';
  if (s.includes('attempt')) return 'DELIVERY_ATTEMPTED';
  if (s.includes('fail') || s.includes('undeliver') || s.includes('review') || s.includes('refused') || s.includes('not accepted')) return 'DELIVERY_FAILED';
  if (s.includes('return') && s.includes('transit')) return 'RETURN_IN_TRANSIT';
  if (s.includes('return') && (s.includes('shipper') || s.includes('merchant'))) return 'RETURNED_TO_SHIPPER';
  ...
  if (s.includes('arrived') || s.includes('station') || s.includes('hub')) return 'ARRIVED_AT_ORIGIN';
  if (s.includes('transit') || s.includes('dispatch') || s.includes('route')) return 'IN_TRANSIT';
  if (s.includes('picked') || s.includes('pickup') || s.includes('assign')) return 'PICKED_UP';
  if (s.includes('book') || s.includes('pending') || s.includes('created')) return 'BOOKED';
