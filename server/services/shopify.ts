...
export class ShopifyService {
  ...
  async streamOrdersByPage(
    shop: string,
    accessToken: string,
    params: { status?: string; created_at_min?: string; updated_at_min?: string },
    ...
    console.log(`[Shopify] Starting streamed paginated fetch from ${shop}...`);

    while (url) {
      pageCount++;
      console.log(`[Shopify] Fetching page ${pageCount}... (${totalFetched} orders processed so far)`);
      try {
        const response = await fetch(url, { headers });
        ...
        await onPage(pageOrders, pageCount);

        if (url) {
          const rateDelay = this.getRateLimitDelay(response);
          await this.sleep(Math.max(rateDelay, 300));
        }
      } catch (error) {
        console.error(`[Shopify] Error on streamed page ${pageCount}:`, error);
        retryCount++;
    ...
    console.log(`[Shopify] Streamed fetch complete: ${totalFetched} total orders in ${pageCount} pages`);
  }

  private async processOrderBatch(
    merchantId: string,
    shopifyOrders: ShopifyOrder[],
    ctx: { minOrderDate: Date; now: Date; storage: any }
  ): Promise<{ newCount: number; updatedCount: number }> {
    const { minOrderDate, now, storage } = ctx;
    ...
    interface PendingNewOrder { createData: any; initialWorkflowStatus: string; tags: string[] }
    interface PendingUpdate { existingOrderId: string; updateData: any; initialWorkflowStatus: string; shopifyOrder: ShopifyOrder; transformedOrder: any; hasCourierStatus: boolean }
    ...
    for (const shopifyOrder of shopifyOrders) {
      ...
      if (orderCreatedAt < minOrderDate) continue;
      ...
      if (existingOrderId) {
        ...
        pendingUpdates.push({ existingOrderId, updateData, initialWorkflowStatus, shopifyOrder, transformedOrder, hasCourierStatus });
      } else {
    ...
    for (let i = 0; i < pendingNewOrders.length; i += BULK_INSERT_SIZE) {
      ...
      try {
        const createdOrders = await storage.createOrdersBulk(batch.map(item => item.createData));
        newCount += createdOrders.length;
        const createdByShopifyId = new Map(createdOrders.map((o: any) => [o.shopifyOrderId, o]));
        ...
        if (roboTagPromises.length > 0) await Promise.allSettled(roboTagPromises);
      } catch (e) {
    ...
    if (pendingUpdates.length > 0) {
      ...
      for (const o of existingOrdersList) existingOrdersById.set(o.id, o);
      ...
      for (let i = 0; i < pendingUpdates.length; i += PARALLEL_UPDATE_SIZE) {
        ...
        for (const result of results) {
          if (result.status === 'fulfilled') updatedCount++;
          else console.error(`[Shopify] Update failed:`, result.reason);
        }
      }
    }
  ...
  async syncOrders(merchantId: string, shopDomain: string, forceFullSync: boolean = false, onProgress?: (processed: number, total: number) => void): Promise<{ synced: number; updated: number; total: number }> {
    ...
    if (isIncremental) {
      ...
      let fsNew = 0, fsUpdated = 0, fsFetched = 0;
      await this.streamOrdersByPage(shopDomain, plainToken, fetchParams, async (pageOrders, pageNum) => {
        const r = await this.processOrderBatch(merchantId, pageOrders, { minOrderDate, now, storage });
        fsNew += r.newCount;
        fsUpdated += r.updatedCount;
        fsFetched += pageOrders.length;
        console.log(`[Shopify] Page ${pageNum} processed: ${r.newCount} new, ${r.updatedCount} updated (${fsFetched} total so far)`);
      });
      console.log(`[Shopify] Full sync complete: ${fsNew} new, ${fsUpdated} updated, ${fsFetched} total`);
      await storage.getDashboardStats(merchantId);
      await storage.updateShopifyStore(store.id, { lastSyncAt: syncStartTime });
      return { synced: fsNew, updated: fsUpdated, total: fsFetched };
    }
