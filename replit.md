...
- **Timezone-Aware Date Filtering**: All date-based queries use the merchant's timezone (stored in `merchants.timezone`, default `Asia/Karachi`) for date boundary calculations. Utility functions in `server/utils/timezone.ts` convert date strings like "2026-02-01" to UTC boundaries aligned to the merchant's timezone (e.g., Feb 1 00:00 PKT = Jan 31 19:00 UTC). This ensures order counts and filtered results match Shopify's timezone-aware reporting. Applied across dashboard stats, orders, workflow counts, analytics, shipments, COD reconciliation, booking logs, product analytics, and the profitability calculator.
