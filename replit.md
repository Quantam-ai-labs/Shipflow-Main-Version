...
- **Store Switcher**: Users with multiple merchant memberships can switch between stores from the sidebar user menu without logging out. `GET /api/auth/memberships` returns all active memberships; `POST /api/auth/switch-merchant` validates and updates the active `merchantId` on the user record. The sidebar shows the current store name and a "Switch Store" submenu (only when 2+ memberships exist). Page reloads on switch to refresh all merchant-scoped data.
