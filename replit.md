...
- **Auto-Calculated Platform Costs**: `ai_usage_logs` table tracks token usage per OpenAI call (service, model, prompt/completion tokens). Cost dashboard auto-computes WA API costs (outbound messages × rate from `cost_rates`) and AI token costs (total tokens × rate). Rates seeded on startup (`whatsapp_api=$0.05/msg`, `ai_tokens=$0.002/1K`). Frontend shows Auto/Manual badges per cost line.
