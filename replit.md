...
- **Print & Logs System**: Native courier airway bills fetched directly from courier APIs (Leopards `slip_link`, PostEx `/airway-bill/{trackingNumber}` API). `shipment_batches` table logs each booking action (batch ID, courier, status, counts). `shipment_batch_items` records individual order results within each batch. `shipment_print_records` tracks print record metadata. Native slip service (`server/services/courierSlips.ts`) handles fetching PDFs from Leopards (via slip_link URL) and PostEx (via Airway Bill API with token auth). Image responses auto-converted to PDF. Multiple courier slips combined via `pdf-lib` for batch downloads. Routes: `GET /api/print/native-slip/:orderId.pdf` (single order native AWB), `GET /api/print/batch-awb/:batchId.pdf` (batch native AWBs combined). Batch Loadsheet PDF still generated locally via `pdf-lib`. Print info available via `GET /api/print/order/:orderId`. Shipments page has "Batch Logs" tab with batch details and PDF actions. Order Details page shows "Shipping & Print" card with Print/Download buttons for booked orders.
