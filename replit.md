...
- **Print & Logs System**: Generates native courier airway bills and batch loadsheets. Booking Logs tab includes per-row Print/Download AWB buttons for successful bookings. Loadsheet Logs tab has date range filtering, text-labeled action buttons (Print AWBs, Download AWBs, Loadsheet), and per-button loading spinners. Batch AWB generation uses parallel fetching (concurrency=5) and caches generated PDFs to disk for instant repeated downloads. Pipeline booking results modal shares a single fetched blob between Download and Print actions.
