...
- **Meta Ads Sales Launcher (Rebuilt)**: Clean architecture from scratch. Only supports SALES objective with 3 creative modes: Upload Image, Upload Video, Existing Post. Backend is single source of truth for all Meta payloads. Modular services: `server/services/meta/salesPayloadBuilder.ts` (separate builders per mode), `salesValidation.ts` (structured preflight validation), `salesDiagnostics.ts` (connection health checks), `salesLaunchService.ts` (stage-by-stage orchestrated launch), `metaErrorParser.ts` (human-readable Meta error messages). Every Meta API call logged to `meta_api_logs` table. Targeting: Pakistan broad (`countries: ["PK"]`), automatic placements, `use_advantage_audience: true`, `bid_strategy: "LOWEST_COST_WITHOUT_CAP"`. Existing post mode disables copy/URL/CTA fields (runs as-is). Publish modes: Validate Only, Create as Draft, Publish Live. Frontend at `client/src/pages/meta/sales-launcher.tsx`. Legacy launcher code retained in `client/src/pages/meta/launcher.tsx` but no longer routed. Bulk Ad Launcher with N creatives × M copy variants combination matrix, **Unified Media Library** (4-tab hub: Local Library with URL + file upload to Meta, Ad Account images/videos, Facebook Page posts/videos, Instagram media — each remote item has "Save to Library" action), Campaign management with status toggle. Legal pages (privacy policy, terms of service, data deletion) at public routes.
