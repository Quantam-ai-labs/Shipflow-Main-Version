...
- **Shipment Tracking**: Records courier tracking and events with universal status normalization. Courier sync scheduler (`server/services/courierSyncScheduler.ts`) uses Leopards bulk `trackMultiple` API (batches of 10 in single requests) and PostEx parallel batches of 10 via `Promise.allSettled` for ~10x faster syncing. Manual sync runs as background job with real-time progress tracking (`processed`/`total`) polled via `GET /api/couriers/manual-sync-progress`.
- **COD Reconciliation**: Tracks payment settlements, including `prepaidAmount`, `codRemaining`, and `codPaymentStatus`. Payment records are immutable after booking. COD payment sync (`POST /api/cod-reconciliation/sync-payments`) runs as background job with real-time progress tracking polled via `GET /api/cod-reconciliation/sync-progress`. Fetches payment details from Leopards (batch via getPaymentDetails) and PostEx (parallel batches of 5, with financial + payment calls concurrent via Promise.all). Auto-marks records as "received" when courier confirms settlement. Schema includes detailed fee breakdown fields: transactionFee, transactionTax, reversalFee, reversalTax, upfrontPayment, reservePayment, balancePayment, courierPaymentStatus, courierSettlementDate, courierPaymentRef, courierSlipLink, lastSyncedAt. Frontend shows expandable rows with fee breakdowns and CSV export with full financial data. Progress bars shown on COD reconciliation and payment ledger pages during sync.
