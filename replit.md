...
- **RoboCall Testing Page**: At `/support/robocall` — testing interface for RoboCall Pakistan (robocall.pk) IVR API. Features: API key verification and balance check, single call form (phone, amount, voice ID, store name, order ID, key1-5), bulk calls (CSV paste or manual rows, max 50 per batch), live call status polling with DTMF response display (1=Confirmed, 2=Cancelled, 3=Callback), session call history table with manual refresh. Backend proxy endpoints at `/api/robocall/*` (verify-key, balance, send, send-bulk, status) — all require authentication.
