...
- **Workflow Transition System**: A 9-stage pipeline (NEW to DELIVERED/RETURN/CANCELLED) with a **strict state machine** enforcing allowed transitions. Both user and system transitions are validated against a hardcoded transition map — invalid transitions are blocked and logged. Only "revert" and "admin_override" actions bypass the guard. Supports Robo-tag processing for status changes and a 12h auto-move for new orders. Pipeline UI includes shipment status sub-tabs for BOOKED, FULFILLED, and RETURN stages (At Origin, In Transit, Picked Up, etc.).
...
- **API-Only Sync System**: Background polling every 30 seconds for incremental Shopify order updates, with manual sync option and live sync status indicator. **Important**: Shopify sync respects ShipFlow's pipeline — orders already in managed workflow states (PENDING, HOLD, READY_TO_SHIP, BOOKED, FULFILLED, DELIVERED, RETURN, CANCELLED) are NOT overridden by Shopify's fulfillment_status. Only truly NEW orders get workflow transitions from Shopify sync. This prevents courier-booked orders from being incorrectly moved to FULFILLED just because Shopify shows them as fulfilled.
...
- **Custom AWB Label Generator**: Client-side HTML label rendering via `client/src/components/awb-label.tsx` with jsbarcode (CODE128) and qrcode libraries. Layout: 3-column top (Ship To, Ship From, Parcel Info) + tracking barcode row + order info row + 2-column bottom (Remarks, Products). Print opens new window with CDN-loaded barcode/QR libs, supports 1/2/3/4 labels per page with live switching. API endpoints: `GET /api/print/label-data/batch/:batchId` and `GET /api/print/label-data/orders?ids=`. Integrated into booking results modal ("Print Labels" button) and BOOKED/FULFILLED pipeline bulk actions.
