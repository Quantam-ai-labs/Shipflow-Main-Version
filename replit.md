...
- **Order & Shipment Management**: Syncs from Shopify, tracks status, allows remarks, and includes a universal status normalization for tracking events. Features an optimized courier sync scheduler with batching and parallel processing. Per-courier sync buttons on Settings > Couriers page (Leopards, PostEx, Sync All). PostEx webhook receiver at `POST /webhooks/postex/status-update` for real-time status pushes. Terminal order re-check sweep runs hourly for recently-finalized orders (last 7 days). Frontend displays universal status labels with raw courier status as tooltip only.
