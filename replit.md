...
- **Agent Chat PWA**: A standalone mobile-optimized PWA at `/agent-chat/:slug` for support agents to manage WhatsApp conversations on their phones. Each merchant gets a unique URL based on their slug (e.g. `/agent-chat/demo-store`). Features: slug-based JWT authentication (no regular login required — agents enter a per-merchant PIN via numeric keypad, server issues a 12h JWT stored in `localStorage` as `agent_chat_token_${slug}`), public `GET /api/agent-chat/merchant-info/:slug` endpoint for merchant name lookup, `POST /api/agent-chat/login` for PIN verification, and all conversation/message/team endpoints under `/api/agent-chat/*` protected by `agentChatAuth` Bearer token middleware scoped to the merchant. Mobile-first view-switching pattern (conversation list vs chat view with back button), dark theme (slate-950), touch-friendly tap targets, quick emoji bar for reactions, formatting toolbar, auto-polling for new messages, 100dvh layout with safe-area support. Separate PWA manifest (`agent-chat-manifest.json`) with `display: standalone`. Install banner with Android `beforeinstallprompt` support and iOS "Add to Home Screen" instructions. Service worker caches static assets for offline shell loading.
